...
describe('deploySkipRule', () => {
  ...
  test('should successfully deploy skip rule with valid input', async () => {
    ...
    expect(result.message).toContain('X-Partner-Key header');
    expect(result.ruleId).toBe('rule-123');
    expect(mockGetOrCreateRuleset).toHaveBeenCalledWith(validInput.zoneId, 'mock-token');
